...
3.  **Renderer Process (`src/`):** React 19 single-page application compiled via Vite. 
    - `src/App.tsx`: Central coordinator managing current views, sidebars, window snaps, tab lists, and layout switches. Listens to IPC shortcut keys to route view-specific actions (e.g., `Ctrl+Shift+A` to activate assistant, or routing Ctrl+`+`/`-`/`0` zoom commands to either active webview, Settings, or Assistant view container zoom).
    ...
    - `src/contexts/SettingsContext.tsx`: Global configuration state synced to the disk using `electronAPI.getStoreValue` / `setStoreValue` (including persistent zoom settings like `assistantZoom` and `settingsZoom`).
...
5.  **CI/CD Build & Release Workflow (`.github/workflows/release.yml`):** Automatically triggered on pushing tags matching `v*`. Runs on `windows-latest` to build the Vite/React frontend, rebuilds native Node modules (`robotjs`) via `electron-rebuild`, packages installers locally with `electron-builder` (`-p never`), and publishes assets (`SideBrowser Setup *.exe`, `SideBrowser *.exe`, and `.blockmap`) using the `softprops/action-gh-release@v2` action.
...
| `copyText(text)` | Renderer ➔ Main | `string` | Copies the specified text to the OS clipboard natively using Electron clipboard module. |
...
3. **Multimodal Inputs & Draft Persistence:**
   ...
   - **Draft Persistence:** Unsent prompt inputs, attached screenshots/images, and uploaded documents are stored in parent `App.tsx` state and passed down as props to `<Assistant />`. This prevents loss of draft prompt contents when toggling tabs or switching browser menus.
4. **Token Tracking & Context Gauges:**
   - Track prompt and completion tokens dynamically from stream payload.
   - Display a context usage gauge dynamically calculated using model name matching heuristics (e.g. 2M for Gemini 1.5 Pro, 128k for GPT-4o and DeepSeek, 32k for Mistral, 8k for local engines).
   - Render remaining balance indicators dynamically aligned above the text input container.
5. **Context Limits & Smart Memory Compression:**
   - When active conversation size exceeds 85% of the model's capacity limit, background summarization is triggered.
   - Old intermediate messages are sent to a real non-streaming query (`ai:query-llm`) in the Main process and replaced with a single summary message: `[System: Conversation history summarized: <summary>]`.
   - Goal Protection: System prompts and the very first user message/assistant reply are permanently protected from compression or pruning to preserve the core query objective.
   - Failover: If summarization fails, a sliding window pruning automatically drops the oldest intermediate messages to ensure continuous chatting.
   - Displays a pulsing loading text "*Optimizing chat memory...*" in the UI while compression is executing in the background.
6. **Screenshot Zoom & Image Preview:**
   - Attached images/screenshots (both in sent messages history and unsent input attachments draft) render with `cursor-zoom-in` and hover opacity indicators.
   - Clicking any image/screenshot launches a high-fidelity full-screen overlay modal. 
   - To prevent small/thumbnail screenshots from rendering too tiny in the center, the modal image uses `w-[calc(100vw-3rem)] max-w-4xl` to stretch and scale up small images inside the viewport.
   - Window screenshots captured via `aiGetOpenWindows` are taken at high-resolution (`1920x1080` instead of the default `150x150` thumbnail size) to ensure complete text legibility and high quality in the zoom preview.
   - The zoom overlay is managed via Framer Motion's `<AnimatePresence>` for smooth fade-in and scale transitions. It renders a clean full-screen darkened background (`bg-black/85`), background blur (`backdrop-blur-sm`), and a close button (`X` icon) with a click-away overlay.
