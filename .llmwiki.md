...
- **Decryption on Demand:** When streaming queries (`ai:query-llm-stream`), the Main process retrieves the encrypted key from disk, decrypts it in memory using DPAPI (Windows) or Keychain (macOS), and queries the LLM provider.
...
| `onAiStreamChunk(cb)` | Main ➔ Renderer | `callback` | Event listener that returns chunks `{ threadId, chunk }`. |
